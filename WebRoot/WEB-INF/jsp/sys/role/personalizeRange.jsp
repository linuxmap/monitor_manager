...
<body>
  	<div style="width:100%;padding: 20px;" class="background">
  		<div style="margin-bottom: 10px;"><span class="jd-title" style="margin-left:0;">${user.userName }</span></div>
...
$(function(){
    ...
    $("#save").on("click",function(){
    	var negativeOrg = new Array();//角色内减少的组织
    	var negativeAsset = new Array();//角色内减少的设备
    	var positiveOrg = new Array();//角色外增加的组织
    	var positiveAsset = new Array();//角色外增加的设备
    	
    	var allNodesTree = treeObj.getNodes();
    	var allNodes = treeObj.transformToArray(allNodesTree);
    	if (allNodes && allNodes.length>0) {
    		for(var k=0; k<allNodes.length; k++) {
    			if (!allNodes[k].extraFlag) {
    				//角色内
    				if (allNodes[k].type == 1 && !allNodes[k].checked) {
    					//减少的组织
    					negativeOrg.push(allNodes[k].id);
    				} else if (allNodes[k].type == 2 && !allNodes[k].checked) {
    					//减少的设备
    					negativeAsset.push(allNodes[k].dbassetId + "_" + allNodes[k].pId);
    				}
    			} else {
    				//角色外
    				if (allNodes[k].type == 1 && allNodes[k].checked) {
    					//增加的组织
    					positiveOrg.push(allNodes[k].id);
    				} else if (allNodes[k].type == 2 && allNodes[k].checked) {
    					//增加的设备
    					positiveAsset.push(allNodes[k].dbassetId + "_" + allNodes[k].pId);
    				}
    			}
    		}
    	}
    	...
    	if (!userId) {
    		layer.msg("请选择用户",{time: 2000, icon:6});
    		return;
    	}
		$.ajax({
			type: "POST",
			url: '${sys_ctx}/role/updatePersonalizeRange.action',
	    	data: {
	    		'roleId':roleId,
	    		'userId':userId,
	    		'negativeOrg' : negativeOrg,
	    		'negativeAsset' : negativeAsset,
	    		'positiveOrg' : positiveOrg,
	    		'positiveAsset' : positiveAsset
	    		},
			dataType:'json',
			cache: false,
			success: function(data){
				if(data.success == "true"){
					...
					var err = data.msg;
					layer.alert("保存失败", {title:'提示信息',icon: 5});
				 }
			},
			error: function(data){
