...
<html>
  ...
  <body style="margin-bottom:20px;">
  	<div id="dlg"  style="padding:10px 20px;" >
        <form id="fm" method="post">
  			...
  			<input name="assetId" id="assetId" value="${vmsAsset.assetId}" type="hidden" />
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" >
            	<tr>
					<td><label><span style="color: red;">*</span>设备类型：</label>
						...
						<input type="hidden" id="loginFlag" name="loginFlag" value="${deviceType.loginflag }"/><!-- 是否有登录信息的标识 -->
                ...
                <tr >
                    ...
                    <td><label><span style="color: red;">*</span>产品：</label>
                    	<select name="productId" id="productId" class="default-one-input" onchange="getModelOptions(this.value);"  data-rule-required="true">
                    			<option value="">请选择</option>
								 <c:forEach items="${currentProductList }" var="product">
                ...
                <tr >
                    <td><label><span style="color: red;">*</span>设备型号：</label>
                    	<select name="modelId" id="modelId" class="default-one-input" onchange=""  data-rule-required="true">
                    		 <option value="">请选择</option>
							 <c:forEach items="${currentModelList }" var="model">
	      ...
	      <fieldset class="bor-form"> 
                ...
                <tr >
                    <td><label>经度：</label>
                    	<!-- camera表 id的隐藏域 -->
                    	<input type="hidden" value="${vmsAsset.assetId }" name="cameraId"/>
               ...
               <fieldset class="bor-form loginChunk"> 
	  				<legend>登录信息</legend> 
		            <table>
                    <td><label>IP地址：</label>
                    	<!-- 登录平台id的隐藏域 -->
                    	<input type="hidden" value="${deviceLoginVO.deviceloginId }" name="deviceloginId"/>
                    	<input type="text" id="ip" name="ip" value="${deviceLoginVO.ip }" title="请输入IP地址">
                    </td>
                    <td style="padding-left:18px;"><label>端口：</label>
                    	<input type="text" id="port" value="${deviceLoginVO.port }" name="port" title="请输入端口"/>
                    </td>
                </tr>
                <tr>
                    <td><label>用户名：</label>
                    	<input type="text" id="loginuser" name="loginuser" value="${deviceLoginVO.loginuser }" title="请输入用户名">
                    </td>
                    <td  style="padding-left:18px;"><label>密码：</label>
                    	<input type="text" id="loginpwd" value="${deviceLoginVO.loginpwd }" name="loginpwd" title="请输入密码"/>
                    </td>
                </tr>
                <tr>
                    <td><label>连接方式：</label>
                    	<select name="connecttype" id="connecttype" class="default-one-input" >
                    			 <option value="">请选择</option>
								 <option value="1" <c:if test="${deviceLoginVO.connecttypeLong  == 1}">selected="selected"</c:if>>SDK</option>
								 <option value="2" <c:if test="${deviceLoginVO.connecttypeLong  == 2}">selected="selected"</c:if>>Onvif</option>
								 <option value="3" <c:if test="${deviceLoginVO.connecttypeLong  == 3}">selected="selected"</c:if>>GB28181</option>
								 <option value="4" <c:if test="${deviceLoginVO.connecttypeLong  == 4}">selected="selected"</c:if>>OCX</option>
						</select>
                    </td>
                    <td  style="padding-left:18px;"><label>连接值：</label>
                    	<input type="text" id="connectvalue" value="${deviceLoginVO.connectvalue }" name="connectvalue" title="请输入连接值"/>
                    </td>
                </tr>
                <tr>
                    <td><label>通道数量：</label>
                    	<input type="text" id="channelcount" name="channelcount" value="${deviceLoginVO.channelcount}" title="请输入通道数量" />
                    </td>
                </tr>                     		                         
            </table>
      	</fieldset>
  ...
  <script type="text/javascript">
	...
	$(function(){
		...
		//初始化加载缩放级别
		var $zoom = $("#zoom");
		var selectedZoom = '${vmsCamera.zoom}';
		for(var i=1; i<22; i++){
			var option = "<option value="+i+">"+i+"</option>";
			$zoom.append(option);
		}
		$("#zoom").find("option[value="+selectedZoom+"]").attr("selected",true);
	});
	...
	layui.use('laydate',function(){
		...
		//时间的设置和校验
		var start = {
			/* min : laydate.now(0,'YYYY年MM月DD日')//自定义最小时间获取日期的当前时间
	...
	var devicetypeId = $("#devicetypeId").val();
	   ...
	   $("#addType").on("click",function(){
	   		layer.open({
				...
				anim: 'scale',
				content: '<lable>类型名称：</lable><input type="text" style="width:150px;height:30px;padding-left:10px;border-radius:3px;" id="typeName"  onblur="validateTypeName()"></br> <lable style="margin-top:20px;display:inline-block;line-height: 28px;height:28px;">是否需要登录信息：</lable><input type="checkbox" style="width:150px;height:30px;padding-left:10px;border-radius:3px;" id="loginflag" >',
				btn: ['确认','取消'],
				yes: function(index) {
	   ...
	   $("#addModel").on("click",function(){
	   		...
	   		if(vendorId && productId){
		   		layer.open({
					...
					anim: 'scale',
					content: '<lable style="width: 70px;text-align: right;display:inline-block;line-height: 28px;height:28px;">厂商：</lable><input readonly="readonly" type="text" style="width:150px;height:30px;padding-left:10px;border-radius:3px;" value='+vendorName+'></br><lable style="margin-top:10px;display:inline-block;line-height: 28px;height:28px;">产品名称：</lable><input readonly="readonly" type="text" style="width:150px;height:30px;padding-left:10px;border-radius:3px;" value='+productName+'></br><lable style="margin-top:10px;display:inline-block;line-height: 28px;height:28px;">型号名称：</lable><input type="text" style="width:150px;height:30px;padding-left:10px;border-radius:3px;" id="modelName" onblur="validateModelName()">',
					btn: ['确认','取消'],
					yes: function(index) {
						var modelName = $("#modelName").val();
						//判断名字是否为空
    ...
    function save() {
  		...
  		var url = '${sys_ctx}/asset/editLoginAsset.action';
