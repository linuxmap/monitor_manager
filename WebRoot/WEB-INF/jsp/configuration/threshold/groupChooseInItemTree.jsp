...
<html>
	...
	<script>
		$(function(){
		debugger;
			$('.layui-layer-title', parent.document).html("选中告警分组");
						  	...
						  	<ul id="leftLi" class="">
						  		...
						  		<c:forEach items="${groupList }" var="item">
						  			<li value="${item.groupId }" data-itemname="${item.name }">
						  				<input type="radio" value="${item.groupId }" name="groupradio" style="border: 0 !important;"/>
						  				<span class="dblinput">
    ...
    function saveData(){
        ...
        layer.confirm('确定要保存该信息吗？', {btn: ['确定', '取消'],icon: 3 }, function(index, layero){//确定按钮回执函数
  			$.ajax({
  				...
  				type : "POST",
  				cache : false,
  				data : {'typeId':deviceTypeId,'ids':ids, 'groupId':groupId},
  				dataType : 'json',
