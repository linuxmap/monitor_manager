  ...
  <body class="default-one-body">
	<div class="table-top" >
		<form action="${sys_ctx}/dictionaryItem/queryDictionaryItem.action" name="frm" id="frm"
			...
			<p class="tab-p">
				<label class="default-one-lable">字典条目名称</label> 
				...
				<c:if test="${!empty list}">
					<c:forEach var="s" items="${list }" varStatus="status">
						<tr class="forsearch">
							...
							<td class="forsearch">
								<c:out value="${s.itemId}"/>
							</td>
							<td class="forsearch">
								<c:out value="${s.itemName}"/>
							</td>
							<td class="forsearch">
								<c:out value="${s.itemStatus}"/>
							</td>
							<td class="forsearch">
								<c:out value="${s.itemDesc}"/>
							</td>
							<td>
								<a class="site-button-method" id="${s.id}" data-method="viewDictionaryItem">查看&nbsp;</a>
								<span name="editSpan" style="text-align:center; display:none;">
									<a class="site-button-method" id="${s.id}" data-method="updateDictionaryItem">|&nbsp;编辑&nbsp;</a>
								</span>
								<span name="copySpan" style="text-align:center; display:none;">
									<a class="site-button-method" id="${s.id}" data-method="copyDictionaryItem">|&nbsp;复制&nbsp;</a>
								</span>
							</td>
						</tr>
					</c:forEach>
			  ...
			  var active = {
				addDictionaryItem: function(){
				      ...
				      parent.layer.open({
				        ...
				        ,maxmin: false
				        ,offset: ''
				        ,content: '${sys_ctx}/dictionaryItem/toOpenDictionaryItem.action?dictionaryId='+dictionaryId+'&dictionaryName='+dictionaryName+'&funtType='+funtType
				        ,end: function(){
			    ...
			    updateDictionaryItem: function(){
				    ...
				    parent.layer.open({
				        ...
				        ,maxmin: false
				        ,offset: ''
				        ,content: '${sys_ctx}/dictionaryItem/toOpenDictionaryItem.action?id='+id+'&funtType='+funtType+'&dictionaryId='+dictionaryId
				        ,end: function(){
				        	if(parent.page_sign==1){refresh();}//刷新页面
				        }
				      });
